...
gsap.to('#cover-pr>#cover_d>div', {height: '100vh', width: '100vw', scrollTrigger: {
    trigger:"#cover-pr",
    end: "+=100%",
...
Ser_.forEach((container) => {
  ...
  tl.to(ser, 1,{
        y:0,
        duration:0.5,
        ease: Power2.out,opacity:1},"-=0.7")
  tl.from(ser2, 1,{
            y:40,
            duration:0.5,
            ease: Power2.out,opacity:0},"-=0.8")
  // tl.to(Ser,1,{
  //   y:40,
  //   duration:0.5,
  //   ease: Power2.out,opacity:1},"-=0.5")
...
ClickForm.staggerFrom(".frm",0.5,
{ y: 100, opacity: 0, duration:0.5, ease:'power2.In' }, "-=0.3"
);
...
function updateFileProgress(e) {
  if (e.lengthComputable) {
    ...
    clippaper.style.strokeDashoffset =
      -clippaperLength + (percentage * clippaperLength) / 100;
